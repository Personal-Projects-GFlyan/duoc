import Link from "next/link";

type CardPropsDesktop = {
    // category: string[],
    id: number,
    title: string,
    localization: string,
    imageSrc: string,
    imageAlt: string;
}

export default function DesktopProjectCard({id, title, localization, imageSrc, imageAlt}: CardPropsDesktop) {
    return(
        <Link href={`/project/${id}`} className="cursor-pointer">
            <article className="relative min-w-[250px] max-w-[250px] min-h-[150px] max-h-[150px] xl:min-w-[300px] xl:max-w-[300px] xl:min-h-[200px] xl:max-h-[200px] rounded-xl overflow-hidden transition hover:scale-105">
                <img src={imageSrc} alt={imageAlt} loading="eager" decoding="async" fetchPriority="low" className="absolute w-[250px] h-[150px] xl:w-[300px] xl:h-[200px] z-10"/>
                <div className="p-5 z-30 absolute bottom-0 max-w-[275px] overflow-clip">
                    <h1 className="text-white font-bold text-[18px]">{title}</h1>
                    <div className="flex gap-1"> 
                        <img src="/localization.svg" alt="Localization Icon" width={7.5} height={5} loading="eager" decoding="async" fetchPriority="low"/>
                        <h2 className="text-[10px] text-white italic">{localization}</h2>
                    </div>
                </div>
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent z-20" />
            </article>
        </Link>
    );
}